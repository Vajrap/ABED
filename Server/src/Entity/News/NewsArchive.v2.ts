...
export class NewsArchive {
  ...
  async loadFromDatabase(): Promise<void> {
    try {
      ...
      for (const record of newsRecords) {
        ...
        if (freshness > 0) {
          const newsRecord: NewsRecord = {
            ...
            currentLocations: new Set(record.currentReach as LocationsEnum[] || []),
            freshness,
      ...
      Report.error("Failed to load news from database:", error as Record<string, unknown> | undefined);
  ...
  // TODO: Should calculate decay and spread at the same time, else we loop twice.
  /**
  ...
  async saveToDatabase(): Promise<void> {
    try {
      ...
      if (newsToInsert.length > 0) {
        await db.insert(newsArchiveTable).values(newsToInsert.map(news => ({
          ...news,
          secretTier: news.secretTier?.toString()
        })));
      }
      ...
      Report.error("Failed to save news to database:", error as Record<string, unknown>);
    }
  }
