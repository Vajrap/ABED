...
class AuthService {
  ...
  async login(credentials: LoginRequest): Promise<AuthResponse> {
    try {
      const response = await restHandler.post<LoginRequest, AuthResponse>("/api/login", credentials);
      return response;
    } catch (error) {
  ...
  async register(userData: RegisterRequest): Promise<AuthResponse> {
    ...
    try {
      const response = await restHandler.post<RegisterRequest, AuthResponse>("/api/register", userData);
      return response;
    } catch (error) {
