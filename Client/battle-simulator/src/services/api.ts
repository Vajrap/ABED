...
export class BattleSimulatorAPI {
  ...
  static async simulateBattle(
    ...
    if (!response.ok) {
      const error = await response
        .json()
        .catch(() => ({ error: "Failed to simulate battle" }));
   ...
   */
  static async analyzeBattle(
    battleResult: SimulateBattleResponse,
    systemPrompt: string,
  ): Promise<AIAnalysis> {
    ...
    const webhookUrl =
      import.meta.env.VITE_N8N_ANALYSIS_WEBHOOK ||
    ...
    try {
      ...
      throw new Error(
        `Failed to parse AI analysis: ${error}. Response: ${cleanedText.substring(0, 200)}`,
      );
    }
  }
   ...
   */
  private static formatAnalysisPrompt(
    battleResult: SimulateBattleResponse,
    systemPrompt: string,
  ): string {
